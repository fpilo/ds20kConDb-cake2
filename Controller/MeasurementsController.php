    ...
    if($fp == null){
		...
		switch($mime){
			...
			case "text/plain":
				...
				if($ffr->foundMatch && $ffr->match == 'csv-md'){
					
					//Create new folder for tmp files storaging
					$basepath = TMP_FILE.DS."multi_dev_meas_";
					$counter = 1;
					...
					foreach($files as $id=>$file){
						...
						$files[$id] = "multi_dev_meas_".$counter."/".$files[$id];
					}
				...
				elseif($ffr->foundMatch &&  $ffr->match == 'csv'){
  ...
  public function add($itemId = null) {
    ...
    $url = Router::url(array('plugin'=>'plupload','controller' => 'plupload', 'action' => 'upload'));
		...
		if ($this->request->is('post')) {
			...
			$tmpMeasurements=$this->request->data['tmpMeasurements'];
			if(!empty($tmpMeasurements)){
				foreach($tmpMeasurements as $dummy => $tmpMeasurement){					
			
					if($tmpMeasurement['status']=='error') continue;
					
					if(isset($tmpMeasurement['fileName'])){
						//User Request, just add filename to database and notify skript to deal with the rest
						$fileName = $tmpMeasurement['fileName'];
					}else{
						echo __("Couldn't find file pointer, abort");
						exit();
					}
										
					if(isset($tmpMeasurement["measurementTags"])){
						foreach($tmpMeasurement["measurementTags"] as $measurementTag){
							$measurementTags[] = $measurementTag;
						}
					}else{
						$measurementTags = array();
					...
					if(isset($tmpMeasurement["measurementSetup"])){
						$measurementSetupId = $tmpMeasurement["measurementSetup"];
					}else{
						$measurementSetupId = null;
					}
					...
					$measurementIds = $mFile->save();
					$this->set("measurementIds",$measurementIds);
					//FP Mettere protezioni se qualcosa va storto!!!
      ...
      //
		...
		//FP Reset temporaneo delle misure storate nella session ... mettere in un altro posto??
		$tmpMeasurements = array();
		$this->Session->write("tmpMeasurements", $tmpMeasurements);
...
#     debug($this->Measurement->validationErrors);
	...
	*/
	...
	public function addTmpMeasurement() {
		
		if($this->RequestHandler->isAjax()) {
		
			$session = $this->request->data['session'];
			$tmpMeasurements = $this->Session->read($session);

			if(empty($tmpMeasurements)) $tmpMeasurement['id'] = 1;
			else $tmpMeasurement['id'] = end($tmpMeasurements)['id'] + 1;

			$tmpMeasurement['fileName'] = $this->request->data['fileName'];			
			$tmpMeasurement['status'] = $this->request->data['status'];
			$tmpMeasurement['itemCode'] = $this->request->data['itemCode'];
			if($tmpMeasurement['status']!='error'){
				$tmpMeasurement['itemId'] = $this->request->data['itemId'];
				$tmpMeasurement['measurementSetup']['id'] = $this->request->data['measurementSetupId'];
				$tmpMeasurement['measurementSetup']['name'] = $this->request->data['measurementSetupName'];
			}
			else{
				$tmpMeasurement['itemId'] = null;
				$tmpMeasurement['measurementSetup']['id'] = null;
				$tmpMeasurement['measurementSetup']['name'] = '';

			}
			
			if(!empty($tmpMeasurement)) {
				$tmpMeasurements[] = $tmpMeasurement;
				$this->Session->write($session, $tmpMeasurements);
			}
		}

		$this->set('tmpMeasurements', $tmpMeasurements);
		$this->render('preview_table', 'ajax');
