	...
	public function getOptions() {
		...
		foreach($options as $option => $value) {
			if($option == "init") {
				...
				$out .= "\t},\n";
			} else if($option == "multipart_params") {
				$out .= $option.": \t{\n";
				...
				$out .= $option.": ".$this->Js->object($value).",\n";
			} else {
				$out .= $option.": '".$value."',\n";
			}
		}
 ...
 */
	public function loadWidget($ui = 'jqueryui', $option = array()) {
		...
		return $this->Html->tag(
			'iframe',
			'',
			array('src' => Router::url($this->settings['widget_url'] . "/" . $ui, false), 'width' => $width, 'height' => $height, 'frameborder' => $frameborder, 'escape' => $escape)
		);
	}
}