<div class="form">
	...
	<div class="row">
		<div class="col-md-4">
			...
			<?php echo $this->Form->input('group_id', array('id' => 'AroSelector', 'div' => false, 'label' => 'Jump to...', 'options' => $aroList, 'empty' => $aroAlias, 'value' => isset($this->params['named']['sel']) ? $this->params['named']['sel'] : ''));?>
	...
	<table class="aclmanager-permissions">
		...
		foreach ($acos as $id => $aco) {
			...
			if($aco['Aco']['id'] == $prevId) continue;
			else $prevId = $aco['Aco']['id'];
			...
			elseif ($ident != $lastIdent) {
				...
				?><tr class='aclmanager <?php echo h($parentAlias) ?>' style="display:none"><?php
			}
...
jQuery(document).ready(function($){
	...
	$("#AroSelector").on('change', function(){
		...
		var sel = $(this).val();
		var page = sel / <?php echo $this->Paginator->params()['limit'] ?>;
		if (page!=="") {
			location.href = "<?php echo $this->Html->url($params);?>/page:" + (parseInt(page)+1) + "/sel:" + parseInt(sel);
		}
	});
