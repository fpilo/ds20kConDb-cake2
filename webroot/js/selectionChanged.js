$(document).ready(function() {			
	...
	$('#tbl').tableSelect({
		...
		onDoubleClick: function(rows) {
			$.each(rows,function(i,row) {
				...
				$.post(
					'<?php echo $this->Html->url(array('controller' => 'items', 'action' => 'selectionChanged')); ?>/',
					//'/cakephp/hephydb/items/selectionChanged/',
					{id:  id, html: html },
					handleSelctionChanged
				);	
	...
	function handleSelctionChanged(table){
		$('#selectedRows').remove();
		if(table.length > 0){
