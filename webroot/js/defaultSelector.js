$(document).ready(function(){	
	subSelects = $(document).find('[parent]');
	$(subSelects).each( function( index ) {
	  if(!HasParent(this)) {
	  	ResetSubSelect(this);
	  	FillSubSelect(this);
	  }
...
$(document).ready(function(){
	...
	joans = $(document).find('[joan]');
	$.each(joans, function( index, elem ) {
	  var joan = $(elem).attr('joan');
	  var e = $(elem);
	  jQuery.data( e, "options", window[joan]);
	  var options = jQuery.data( e, "options" );
	});	
	...
	ssans = $(document).find('[ssan]');
	$.each(ssans, function( index, elem ) {
	  SetSelectedValues(elem, GetSelectedOptions(elem));
	});
...
function FillSubSelect(subSelect)
{
	options = GetAllOptions(subSelect);
	for (var k in project) {
		$(subSelect).append(project[k]);
	}
}

/*
 * Get all possible options for a SubSelect  
 */
function GetAllOptions(subSelect)
{
	var joan = $(subSelect).attr('joan');
	return window[joan];
}

function GetSelectedOptions(subSelect)
{
	  var ssan = subSelect.attr('ssan');
	  return GetArray(ssan);	  
}

function HasParent(subSelect)
{
	var hasParent =$(subSelect).attr('parent');
...
// alle <option>s des sub-<select> entfernen
function ResetSubSelectById(subSelectId)
{
...
function SetSelectedValues(subSelect, arr) {
...
function SetSelectedValuesById(subSelectId, arr) {
...
function UpdateSubselect(subSelect) {
	//var elem = document.getElementById('manufacturer_id');
	
	if(elem.selectedIndex >= 0) {
		var childAppended = false;
		
		InitChildSubSelect(subSelect);
        //InitSubSelect('item_subtype_version_id');
        //InitSubSelect('item_subtype_id');
